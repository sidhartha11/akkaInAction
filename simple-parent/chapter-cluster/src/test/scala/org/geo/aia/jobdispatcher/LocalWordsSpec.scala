...
import aia.cluster.words.StopSystemAfterAll
...
with ImplicitSender {
  ...
  "The words system" must {
    ...
    expectMsg(JobSuccess("test2" , Map("this" -> 4, "is" -> 3 , "a" -> 2,
        "test" -> 2)))
    expectNoMessage
    }
    
    "count many occurrences of words in a text" in {
      val words = List("this is a test" , "this is a test" , "this is", "this")
      receptionist ! JobRequest("test3" , ( 1 to 100 ).map(i => words ++ words)
          .flatten.toList)
     expectMsg(JobSuccess("test3", Map("this" -> 800 , "is" -> 600 , "a" -> 400,
         "test" -> 400)))
     expectNoMessage
    }
    
    "continue to process a job with intermittent failures" in {
      // the failure is simulated by a job worker throwing an exception on finding
      // the word FAIL in the text
