...
/**
 * https://medium.com/akka-for-newbies/routers-5a501cdf616d
 */
...
class GetLicense(pipe: ActorRef, initialServiceTime: FiniteDuration = 0 millis)
  ...
  println("GetLicense initialized: id= %s, servicetime=%s".format(id,serviceTime))

  def receive = {
    case init: SetService => {
      println("got initial SetService message")
      id = init.id
      serviceTime = init.serviceTime
      Thread.sleep(100)
    }
    case msg: PerformanceRoutingMessage => {
      println("got PerformanceRoutingMessage:" + msg)
